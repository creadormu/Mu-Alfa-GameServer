...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_STARTARENA:
			...
			break;
		case COMMAND_MARRY_NEW:
			this->SetMarry(lpObj,argument);
			break;
		case COMMAND_ACCEPT_MARRY:
			gMarry.CommandCore(lpObj,1);
			break;
		case COMMAND_DIVORCE:
			gMarry.CommandCore(lpObj,2);
			break;
		case COMMAND_SETLVL:
		...
		case COMMAND_PKSETLVL:
            ...
            break;
		case COMMAND_FORCE_DIVORCE:
			gMarry.CommandCore(lpObj,3);
			break;
		case COMMAND_TRACE_MARRY:
			gMarry.CommandCore(lpObj,4);
			break;
		case COMMAND_MARRY_STATUS:
			gMarry.CommandCore(lpObj,5);
			break;
		default:
			return 0;
	}
...
bool CCommandManager::CommandPost(LPOBJ lpObj,char* arg) // OK // AGREGANDO POST VIP
{
...
bool CCommandManager::SetMarry(LPOBJ lpObj,char* arg) // OK
{
	if(gMarry.Enabled == TRUE)
	{	
		gCommandManager.ProposeMarry(lpObj,arg);
		return TRUE;
	}
}

void CCommandManager::ProposeMarry(LPOBJ lpObj,char * Wife)
{
	int lpTargetID = ObjYola.gObjGetIndex(Wife);
	
	int lpObjSex = gMarry.GetSex(lpObj);
					
	int lpTargetSex = gMarry.GetSex(&gObj[lpTargetID]);
	...
	if(gMarry.GetPos(lpObj) > 0 )
	{		
		if(gMarry.GetPos(lpObj) > 0 && gMarry.GetPos(&gObj[lpTargetID]) > 0 )
		{
			...
			{
				gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"You cant marry with some one with the same sex!");}
	
		}
		else
		{
			gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Propose] %s incorrect Position!",gObj[lpTargetID].Name);
			gNotice.NewNoticeSend(gObj[lpTargetID].Index,0,0,0,0,0,"[Propose] %s You position is %d,%d!",gObj[lpTargetID].Name,gMarry.AcceptX,gMarry.AcceptY);
		}
		
	}
	else
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Propose] %s You position is %d,%d!",lpObj->Name,gMarry.ProposeX,gMarry.ProposeY);
		gNotice.NewNoticeSend(gObj[lpTargetID].Index,0,0,0,0,0,"[Propose] %s You position is %d,%d!",gObj[lpTargetID].Name,gMarry.AcceptX,gMarry.AcceptY);
	}
}

void CCommandManager::AcceptMarry(LPOBJ lpObj)
{
	int lpTargetID = ObjYola.gObjGetIndex(lpObj->MarryCharacterInfo.MarryTarget);

	if(lpTargetID >= 0)
	{
		if(lpObj->MarryCharacterInfo.MarrySeconds >= gMarry.MarryMaxTime) 
		{
		...
		if (lpObj->MarryCharacterInfo.Married == 0)
		{
			...
			GCFireworksSend(lpObj,gMarry.AcceptX,gMarry.AcceptY);
			...
			gMarry.SendInfo(lpTargetID);
			gMarry.SendInfo(lpObj->Index);
		
			//gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(506));
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"[MarrySystem]");

			//gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(507),gObj[lpTargetID].Name,lpObj->Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,"%s and %s Married",gObj[lpTargetID].Name,lpObj->Name);

		}
		else
		{
			gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Accept] You are already married!");
		}

	}
	else
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Accept] Character %s doesnt exist or is disconnected",lpObj->MarryCharacterInfo.MarryTarget);
	}
}

void CCommandManager::TraceMarry(LPOBJ lpObj)
{
	int lpTargetID = ObjYola.gObjGetIndex(lpObj->MarryCharacterInfo.MarriedName);
	...
	{	
		...
		{
			gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"%s is inside of one event, you cant warp there!",lpObj->MarryCharacterInfo.MarriedName);
			return;
		}
	// - REQ-Gens
	
		if(lpObj->Level < gMarry.MapIndex[lpTargetMap])
		{
			gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"You need level %d for teleporting",gMarry.MapIndex[lpTargetMap]);
			return;
		}
...
void CCommandManager::DivorceMarry(LPOBJ lpObj)
{	
	int lpTargetID = ObjYola.gObjGetIndex(lpObj->MarryCharacterInfo.MarriedName);
	...
	if(lpTargetID <= 0)
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Divorce] %s is disconnected",lpObj->MarryCharacterInfo.MarriedName);
		return;
	}

	if(gMarry.CostDivorce > lpObj->Money )
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Divorce] You need %d zen to divorce",gMarry.CostDivorce);
		return;
	}

	lpObj->Money -= gMarry.CostDivorce;
	GCMoneySend(lpObj->Index,lpObj->Money);				

	gObj[lpTargetID].Money += gMarry.CostDivorce;
	...
	gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Divorce] You get divorced, your Zen is decreased by %d",gMarry.CostDivorce);

	gNotice.NewNoticeSend(gObj[lpTargetID].Index,0,0,0,0,0,"[Divorce] You get divorced, your Zen is increase by %d",gMarry.CostDivorce);
...
void CCommandManager::ForceDivorceMarry(LPOBJ lpObj)
{
	int lpTargetID = ObjYola.gObjGetIndex(lpObj->MarryCharacterInfo.MarriedName);
	...
	if(gMarry.CostForceDivorce > lpObj->Money )
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[ForceDivorce] You need %d zen to force the divorce",gMarry.CostForceDivorce);
		return;
	}
	...
	lpObj->Money -= gMarry.CostForceDivorce;
	GCMoneySend(lpObj->Index,lpObj->Money);		
...
void CCommandManager::StatusMarry(LPOBJ lpObj)
{
	if(gMarry.Enabled == 1)
	{
		if(lpObj->MarryCharacterInfo.Married == 1)
